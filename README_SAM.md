...
```bash
ma_public_api$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ma_public_api$ sam local start-api
ma_public_api$ curl http://localhost:3000/classify_digit
```
...
```bash
ma_public_api$ sam logs -n InferenceFunction --stack-name ma_public_api --tail
```
...
```bash
aws cloudformation delete-stack --stack-name ma_public_api
```
